...
logging.basicConfig(level=logging.INFO #设置日志输出格式
                    ,filename="../../../OCR_LOGGING_ORT.log" #log日志输出的文件位置和文件名
...
def main_evaluation(p, default_evaluation_params_fn, validate_data_fn, evaluate_method_fn, show_result=True,
    ...
    if show_result:
        sys.stdout.write("OCR_models_ox_pseORdb_bs1 result : ")
        accs = resDict['method']
        sys.stdout.write("precision : {:.2f}%, recall : {:.2f}%, hmean : {:.2f}%.".format(accs["precision"]*100,accs["recall"]*100,accs["hmean"]*100))
        # sys.stdout.write(json.dumps(resDict['method']))
        logging.info("OCR_models_ox_pseORdb_bs1 result: precision : {:.2f}%, recall : {:.2f}%, hmean : {:.2f}%.".format(accs["precision"]*100,accs["recall"]*100,accs["hmean"]*100))
