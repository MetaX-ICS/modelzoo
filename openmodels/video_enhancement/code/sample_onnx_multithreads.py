...
from common import get_params, preprocess_test, get_FPS_multithread, get_FPS_multithread_v1, \
     check_accuracy, get_EP, model_convert, check_model
import logging
logging.basicConfig(level=logging.INFO #设置日志输出格式
                    ,filename="./VDE_LOGGING_ORT.log" 
...
def main(modelname,batchsize,precision, task="normal", modelfile="./",EP="maca", th_num="8"):
    ...
    image_list = preprocess_test(params['image_T4'])#[:2]
    ...
    if task != "fpsclosed":
        ...
        if task == "fpsonly":
            print("videoenhance_{}_bs{}_prec{} FPS : {:.3f}".format(modelname.replace("/", "_") ,batch_size, precision, FPS))
            logging.info("videoenhance_{}_bs{}_prec{} FPS : {:.3f}".format(modelname.replace("/", "_") ,batch_size, precision, FPS))
            return
