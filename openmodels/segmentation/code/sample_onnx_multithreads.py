...
def main(modelname,batchsize,precision, task="normal", modelfile="./",EP="maca", th_num="8"):
    ...
    if task != "fpsclosed":
        ...
        if task == "fpsonly":
            print("segmatation_{}_bs{}_prec{} FPS : {:.3f}".format(modelname.replace("/", "_") ,batch_size, precision, FPS))
            logging.info("segmatation_{}_bs{}_prec{} FPS : {:.3f}".format(modelname.replace("/", "_") ,batch_size, precision, FPS))
            return
