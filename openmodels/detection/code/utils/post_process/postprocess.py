...
from .sm_getbboxes import get_shanma_bboxes
from .pd_getbboxes import get_ppyoloe_pd_bboxes, get_ppyolo_pd_bboxes, get_yolov3_pd_bboxes
...
def get_bboxes(type, outputs, batch_size, params, start_idx, contents):
    if "vgg16_ssd" in type:
        return eval(type)(outputs, batch_size, params, start_idx)
    elif "pd_" in type:
        return eval(type)(outputs, batch_size, params, contents)
    else:
...
def comute_mAP_coco(preds, params, save_path, iters):
    ...
    pred_jsonpath = "./predictions_{}.json".format(str(uuid.uuid1())[:8])  # predictions json
...
def comute_mAP_voc(preds, params, save_path, iters):
    ...
    inputs = preprocess_test(params['image_T4'])[:iters]
    ...
    for class_index, class_name in enumerate(class_names):
        ...
        if not os.path.exists(save_path+"/voc_results_{}".format(str_uuid)):
            print("create a result path")
            os.mkdir(save_path+"/voc_results_{}".format(str_uuid))

        prediction_path = save_path+"/voc_results_{}/det_test_{}.txt".format(str_uuid,class_name)
...
def pd_xyxy2xywh(bbox, size):    
    ### clip  需要做截断
    y = np.copy(bbox)
    y[:, 0] = bbox[:,0].clip(0., size[0])
    y[:, 1] = bbox[:,1].clip(0., size[1])
    y[:, 2] = bbox[:,2].clip(0., size[2]) - y[:,0]
    y[:, 3] = bbox[:,3].clip(0., size[3]) - y[:,1] 
