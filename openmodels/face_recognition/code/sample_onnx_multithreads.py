...
def main(modelname,batchsize,precision, task="normal", modelfile="./",EP="maca", th_num="8"):
    ...
    batch_size = int(batchsize) if "batch_size" not in params.keys() else int(batchsize)*params["batch_size"]
    precision = precision
    th_num = int(th_num)
    FPS = 0

    image_list = preprocess_test(params['image_T4'])#[:100]
    image_list = image_list if params['image_T4'] != "./data/lfw/pairs.txt" else image_list[1:]
    ...
    if task != "fpsclosed":
        ...
        if task == "fpsonly":
            print("facerecongition_{}_bs{}_prec{} FPS : {:.3f}".format(modelname.replace("/", "_") ,batch_size, precision, FPS))
            logging.info("facerecongition_{}_bs{}_prec{} FPS : {:.3f}".format(modelname.replace("/", "_") ,batch_size, precision, FPS))
            return
